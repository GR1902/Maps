...
├── data/
│   ├── teams.json             649 club/venue records, keyed by league → team code
│   ├── fixtures.json          2851 fixtures, keyed by league, with matchday numbers
...
| UEFA Champions League | Play-off round (MD1) + League Phase MD1 (MD2, complete: 18/18), 25 ties/38 legs (18 Aug–10 Sep) | High | Wikipedia's mirror of uefa.com's fixture calendar, cross-validated by independently converting each kickoff's two given timezones to UTC and confirming they agreed; 2026-08-24: League Phase draw set for 27 Aug — nothing to add until the draw happens. **2026-08-31:** draw held 27 Aug in Monaco (36-team single table); added confirmed, time-verified MD1 (8-10 Sep) fixtures for our 7 tracked play-off winners, stored as matchday 2 — treat this league's matchday numbering as competition-internal (1=play-off round, 2=League Phase MD1, ...), not UEFA's own numbering. **2026-09-01:** completed League Phase MD1 — added the other 12/18 fixtures and 24 new clubs (23 reused from their domestic-league entries via the marker-merge convention plus a country tag; Shakhtar Donetsk newly added — they play League Phase home fixtures at Stamford Bridge, London, not Donetsk, confirmed via 4 independent sources: Sky Sports, Al Jazeera, Chelsea FC and Shakhtar FC's own sites). Every kickoff time cross-checked against 2+ independent sources (Wikipedia's league-phase article + search-engine summaries citing UEFA's own 18:45/21:00 CET two-slot pattern, plus ESPN's schedule tool for 10 Sep specifically). League Phase MD2 (13-14 Oct) not attempted — too far out for times to be published yet |
| UEFA Europa League | Play-off round, MD1 return legs (27 Aug) + League Phase MD1 (MD2, complete: 18/18, 16-17 Sep) | Medium-High — League Phase MD1 cross-checked 2+ sources per kickoff slot; play-off return legs medium (see below) | **2026-08-24/31:** play-off return legs (27 Aug) added, 12/12, 2+ sources per fixture; first legs (20 Aug) never added — no kickoff-time source found, and now historical. **2026-09-01:** added League Phase MD1 in full — 18/18 fixtures, 24 new clubs (22 reused from domestic/other-competition entries via the marker-merge convention plus a country tag; 2 genuinely new — Stade Rennais, Roazhon Park/Rennes coordinates confirmed via Wikipedia infobox, no crest found on Wikimedia so falls back to the plain marker; CD Torreense, who play European home games at Estádio Algarve, Faro — 300 km from their own Torres Vedras ground, which doesn't meet UEFA's requirements, confirmed via 3 independent Portuguese sources). Kickoff times cross-checked 2+ sources per slot, same 18:45/21:00 CET two-slot pattern as Champions League |
| UEFA Conference League | Play-off round (MD1), 13/61 legs (20-27 Aug) + League Phase MD1-3 (MD2-4, complete: 54/54, 15 Oct–5 Nov) | Play-off legs: medium (2+ sources per added leg, rest withheld). League Phase MD1-3: **user-supplied screenshots of uefa.com's own fixtures page** — the most direct source available, but see the timezone caveat below | 2026-08-24/31: play-off round partial (8/48 legs corroborated), league-phase draw held 28 Aug. **2026-09-01, first pass:** automated research hit a wall — uefa.com unreachable via WebFetch (403/503 every attempt, including the user's own direct fixtures-results link) and via a real browser tab (page rendered blank, bot protection); Wikipedia had pairings but not kickoff times for League Phase MD1, and unlike Champions/Europa League's clean two-slot 18:45/21:00 CET pattern, Conference League appeared to use 3+ different slots from the partial data found, so times weren't safely inferable — nothing added that pass. **Second pass, same day:** the user supplied direct screenshots of uefa.com's fixtures page for MD1 (15 Oct), MD2 (22 Oct) and MD3 (5 Nov), each showing every fixture's local kickoff time (17:45 / 15:30 / 20:00 shown). Added all 54 (18×3) fixtures plus 9 new clubs (reused from their existing europa_league entries — Trabzonspor, Kairat, CSKA Sofia, Mjällby, Egnatia, Kauno Žalgiris, Thun, Sint-Truidense, Iberia Tbilisi — via the marker-merge convention). **Timezone caveat:** the screenshots' displayed times were converted to UTC by subtracting 1 hour (i.e. read as UK local time, one hour behind CEST which is still in effect on these Oct/Nov dates) — chosen because that conversion exactly reproduces the 16:45/19:00 UTC two-slot pattern already independently cross-validated for Champions/Europa League's own MD1s, and matched 4 of the 5 fixtures already in our data from the Aug 31 pass. The 5th (Copenhagen–Braga) conflicted — the earlier pass had it at 18:00 UTC, the new screenshot-derived reading gives 19:00 UTC — resolved in favour of the new direct-screenshot source for consistency within one single-source matchday, since the two readings can't both be right and re-deriving one match in isolation from a different source than the other 17 risked a worse inconsistency. This is the best available reading, not a certainty — worth a spot-check against uefa.com directly if reachable later |
